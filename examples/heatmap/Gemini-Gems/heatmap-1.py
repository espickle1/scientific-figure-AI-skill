# v1.0 | Heatmap | biolog_output.csv | All columns standardized
import pandas as pd
...
# Calculate max absolute value for symmetric color scale around 0
max_abs = np.max(np.abs(df_norm))
...
ax.set_xlabel('Column', fontsize=CONFIG["label_size"], bbox=label_bbox)
ax.set_ylabel('Row', fontsize=CONFIG["label_size"], bbox=label_bbox)
...
cax.tick_params(labelsize=CONFIG["tick_size"])
