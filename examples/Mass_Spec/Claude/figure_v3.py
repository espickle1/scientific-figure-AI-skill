...
CONFIG = {
    ...
    "out_png": "/mnt/user-data/outputs/mass_spec_figure_v3.png",
    "out_svg": "/mnt/user-data/outputs/mass_spec_figure_v3.svg",
}
...
# ── BAR CHART ─────────────────────────────────────────────────────────────────
# Deduplicate: keep max AUC per mass value
df_dedup = df.groupby("mass", as_index=False).apply(
    lambda g: g.loc[g["AUC"].idxmax()]
).reset_index(drop=True)

order = np.argsort(df_dedup["mass"].values)
x_sorted = df_dedup["mass"].values[order]
y_sorted = df_dedup["AUC"].values[order]

# Draw as vertical lines (stem-like) from y=0
ax_bar.vlines(x_sorted, 0, y_sorted, linewidth=0.8, color="black", zorder=2)

ax_bar.set_ylabel("AUC", labelpad=4, fontsize=8)
ax_bar.set_title(
    "Peptidoglycan Fragment Abundance by Variations of Structure",
    fontsize=16, fontfamily=FONT, pad=8
)
...
ax_bar.set_yticks([t for t in yticks if t != 0])
...
for cat in cats:
    vals = df_dedup[cat].values[order].astype(float)
...
ax_dot.set_xlabel("Fragment mass (Daltons)", fontsize=8, labelpad=3)
